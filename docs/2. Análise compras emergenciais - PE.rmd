...
soma_mun_item_contr <- item_contrato %>%
  group_by(cd_municipio_ibge) %>% 
  summarise(soma_vl_item_contrato = sum(vl_item_contrato*qt_itens_contrato), soma_qt_itens_contrato = sum(qt_itens_contrato)) %>%
...
soma_mun_item_contr %>%
  ...
  left_join(select(orgaos, id_orgao, cd_municipio_ibge, nome_municipio)) %>%
  select(nome_municipio, soma_vl_item_contrato, pop, casos_acumulado, obitos_acumulado, lic_concluidas, valor_sobre_pib, valor_sobre_pop, casos_sobre_hab, obitos_sobre_hab) %>%
  unique() %>%
  arrange(desc(as.numeric(soma_vl_item_contrato))) %>%
